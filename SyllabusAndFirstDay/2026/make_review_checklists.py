...
def checklist(row, all_rows, hws):
    ...
    any_hw = False
    for hw, vis, review_by, due, covers, problems in hws:
        goes_live_now = (prev_class is None and vis <= d) or (prev_class is not None and prev_class < vis <= d)
        if goes_live_now:
            any_hw = True
            lines += [f"- [ ] **HW{hw:02d} goes live {fmt(vis)}; review by {fmt(review_by)}** "
...
def main():
    ...
    sched = ["# Review schedule, whole semester (PHY 317 F2026)", "",
             ...
             "problems reviewed by THE DAY AFTER the assignment goes live.", ""]
    for row in rows:
