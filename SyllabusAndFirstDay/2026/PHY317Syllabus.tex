...
\newthought{Classical mechanics is where physics learned} to be
physics. Newton's laws are two courses behind you; this course is
about what they become when the forces are real (air resistance,
magnetic fields, springs that are driven and damped), when there are
many bodies, and when you stop writing $\mathbf{F} = m\mathbf{a}$
altogether and ask instead which path a system \emph{would} take out of
all the paths it could. That last question -- the calculus of variations
and Lagrangian mechanics -- is the hinge of the course and the way
every theory you meet after this one is written down. We finish with
orbits, rotating frames, coupled oscillators, and chaos: the discovery
that a completely deterministic pendulum can be unpredictable.
