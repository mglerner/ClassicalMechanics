...
HW_RE = re.compile(r"\bHW\d|homework", re.I)   # graded-set solutions
...
.danger { background: #f8d7da; border: 1px solid #d99; padding: 6px 9px;
          font-size: 13px; margin: 0 0 4px 0; }
.sol .today { font-weight: bold; color: #111; font-style: normal; }
...
def render(n, date, topic, rows, groupwork, pcci, sols, smap, today):
    ...
    # A solution PDF covers a whole chapter, so most of its pages carry other
    # days' problems -- and the homework sets carry answers that are not due
    # yet. Sort today's pages first and say on every page what else is on it,
    # because these get screenshotted into the deck and projected.
    for pg in sols:
        probs = None
        for (sub, page), plist in smap.items():
            if sub in pg["file"].lower() and page == pg["page"]:
                probs = plist
                break
        pg["probs"] = probs
        pg["mine"] = [q for q in (probs or []) if q in today]
        pg["others"] = [q for q in (probs or []) if q not in today]
    ranked = sorted(sols, key=lambda g: (g["hw"], not g["mine"],
                                         g["file"], g["page"]))
    ...
    if not sols:
        ...
        o.append('<p class=warn><b>Instructor copy.</b> Solutions-manual '
                 'excerpts. Never posted, and check the banner on a page '
                 'before projecting it.</p>')
        for pg in ranked:
            cap = f'{pg["file"]}, page {pg["page"]}'
            o.append('<div class=sol>')
            if pg["hw"]:
                o.append('<p class=danger><b>Homework solutions.</b> These '
                         'answers are not due yet. Do not project this page.</p>')
            elif pg["others"]:
                o.append('<p class=danger>Also on this page, and NOT today\'s: '
                         + esc(", ".join(pg["others"]))
                         + '. Do not project it whole.</p>')
            elif pg["probs"] is None:
                o.append('<p class=danger>Page contents not mapped -- add a '
                         '<code>Solutions:</code> line to the prep notes. '
                         'Assume it carries other days\' answers.</p>')
            if pg["mine"]:
                o.append('<p class=cap>Today: <span class=today>'
                         + esc(", ".join(pg["mine"])) + '</span></p>')
            o.append(f'<p class=cap>{esc(cap)}</p>'
                     f'<img src="{esc(pg["src"])}" alt="{esc(cap)}"></div>')
...
def solutions_map(lines):
    """`Solutions: in-class#1 = 3.5, 3.10; look-at#1 = 3.7` -> {(sub, page): [probs]}.

    Optional. Without it the sheet still shows every page, but it cannot say
    which problems are on which page, so it cannot warn about the ones that
    are not today's.
    """
    ...
    for entry in raw.split(";"):
        ...
        out[(sub.strip().lower(), int(page))] = [
            q.strip() for q in probs.split(",") if q.strip()]
    return out


def todays_problems(rows, groupwork):
    """Everything this day legitimately touches: groupwork, PCCI, any row's problems."""
    today = set(groupwork)
    for _a, _b, _m, _mode, _src, text in rows:
        today.update(re.findall(r"\b(\d+\.\d+)\b", text))
    return today
...
def solution_pages(pack_dir):
    ...
    for pdf in sorted(pack_dir.glob(SOL_GLOB)):
        ...
        for png in sorted(out_dir.glob(stem + "-*.png")):
            ...
            pages.append({"src": f"03-answers-files/{png.name}",
                          "file": pdf.name,
                          "page": int(n.group(1)) if n else 0,
...
def main(only=None):
    ...
    for n, date, path in M.pack_files():
        ...
        html = render(n, date, M.topic(lines, path), rows, gw,
                      pcci_number(rows), sols, solutions_map(lines),
                      todays_problems(rows, gw))
